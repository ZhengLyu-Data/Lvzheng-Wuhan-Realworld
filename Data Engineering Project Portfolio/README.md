...
- The following are six real company data engineering projects showcasing data cleaning, SQL analysis, and visualization. They are designed to support teaching, interviews, and portfolio presentation.
    * 以下为六个真实公司数据工程项目的主题与所用工具，涵盖数据清洗、SQL 分析与可视化展示，支持面试与作品集展示。
...
| 05 | NASA Spark IoT Sensor Pipeline<br>美国 NASA 物联网传感器故障检测流程分析 | Fault detection and classification based on industrial sensor logs<br>基于NASA工业传感器数据的故障检测与分类分析流程 | Python, PySpark, Seaborn, Matplotlib |
| 06 | Netflix Cloud ETL Project Pipeline<br>美国Netflix 云端数据处理与 ETL 管道构建流程分析 | Cloud-based ETL pipeline for structured real estate transactions<br>基于美国Neflix显著降低原始日志数据的处理延迟数据清洗与ETL管道构建流程 | Python，PySpark，seaborn, Matplotlib |
